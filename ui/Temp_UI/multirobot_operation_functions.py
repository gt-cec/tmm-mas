...
#     """
#     Selects a specific row from processed robot data using a string robot ID.
...
#     :return: The selected row dictionary or None if an error occurs.
#     """
#     try:
#         return processed_data[robot_id][row_index]  # Direct access
#     except KeyError:
#         print(f"❌ Robot {robot_id} not found in processed data!")
#     except IndexError:
#         print(f"❌ Row index {row_index} out of range for {robot_id}.")
#     return None  # Return None if the key is missing or index is invalid
...
#         print(f"❌ Robot ID '{robot_id}' not found in processed data!")
#         print("Available robot IDs:", list(processed_data.keys()))  # Debugging
#         return None
    
#     robot_data = processed_data[robot_id]

#     if not isinstance(robot_data, list):
#         print(f"❌ Data for {robot_id} is not a list. Found type: {type(robot_data)}")
#         return None

#     if row_index < 0 or row_index >= len(robot_data):
#         print(f"❌ Row index {row_index} is out of range for {robot_id}.")
#         print(f"✅ {robot_id} has {len(robot_data)} rows.")  # Show actual list size
#         return None

#     return robot_data[row_index]  # Safe access
...
#         print(f"❌ Expected processed_data to be a dictionary, but got {type(processed_data)}")
#         return None
...
#         print(f"❌ Data for {robot_id} is not a dictionary. Found type: {type(robot_data)}")
#         return None

#     # Assuming each 'row' in the dictionary has a numerical index or a unique key
#     if row_index not in robot_data:
#         print(f"❌ Row index {row_index} is not found for {robot_id}.")
#         print(f"✅ Available keys for {robot_id}: {list(robot_data.keys())}")  # Show actual keys
#         return None
...
#     """
#     robot_id = f"{robot_number}"  # Map 1 → quad1, 2 → quad2, etc.
