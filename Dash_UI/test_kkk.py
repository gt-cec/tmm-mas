...
# # #         feature_alerts.append("Encountering rough terrain." if rmm_terrain == 0 else "Terrain has cleared.")
# # #     # --- END TERRAIN CHECK ---
...
# # #         details_msg = f"Route recalculated.{pkg_id_text} Current Position (X{x:.0f}, Y{y:.0f} in {quadrant}). {combined_info}".strip()
# # #     elif state == 'carrying':
# # #         status_key = 'on_track'
# # #         pkg_id = next((p['id'] for p in all_packages if p.get('carried_by') == robot_id), "N/A")
# # #         plan = robot_data.get('plan', [])
# # #         dest_x, dest_y = (plan[-1][0], plan[-1][1]) if plan else (x, y)
# # #         details_msg = f"Transporting {pkg_id} to Position (X{dest_x:.0f}, Y{dest_y:.0f}). Currently at Position (X{x:.0f}, Y{y:.0f} in {quadrant}). {feature_text}".strip()
# # #     elif state == 'moving':
# # #         status_key = 'on_track'
...
# # #         details_msg = f"Moving to acquire {pkg_id} at Position (X{goal_x:.0f}, Y{goal_y:.0f}). Currently at Position (X{x:.0f}, Y{y:.0f} in {quadrant}). {feature_text}".strip()
# # #     elif state == 'waiting':
# # #         status_key = 'stationary'
# # #         details_msg = f"Robot is awaiting task at Position (X{x:.0f}, Y{y:.0f} in {quadrant}). {combined_info}".strip()
# # #     elif state == 'all_tasks_complete':
# # #         status_key = 'on_track' # Use 'on_track' style (green)
# # #         details_msg = f"All tasks complete. Robot is at Position (X{x:.0f}, Y{y:.0f} in {quadrant}). {feature_text}".strip()
# # #     else:
# # #         status_key = 'stationary'
# # #         details_msg = f"Robot in unknown state '{state}' at Position (X{x:.0f}, Y{y:.0f} in {quadrant}). {combined_info}".strip()
...
# # #         ], style={'display': 'flex', 'alignItems': 'center', 'fontSize': '1.GET', 'fontWeight': 'bold'})
# # #     )
...
# # #                     message_text = f"Package {pkg_id} discovered by {discovered_by}."
# # #                     if discovered_by != 'N/A' and discovered_by == assigned_to:
# # #                          message_text += f" {discovered_by} is en route to collect it."
# # #                     elif discovered_by != assigned_to and assigned_to != 'N/A':
# # #                         message_text += f" Assigned to {assigned_to}."
...
# # #     app.run(debug=False, host='0.0.0.0', port=9235)
...
# #                 traces['y'].append(r['y'])
# #                 traces['texts'].append(r['id'][-1])
# #                 traces['hovers'].append(f"{r['id']} State: {r['state']} Pos: ({r['x']:.1f}, {r['y']:.1f}) Q: {r['Quadrant']}")
...
# #         'on_track': {'text': 'ON TRACK', 'color': '#e0f5e1', 'icon': '✅', 'class_suffix': 'on-track'},
# #         'stationary': {'text': 'STATIONARY', 'color': '#fff9c4', 'icon': '⏸️', 'class_suffix': 'stationary'},
# #         # --- CHANGE 1: New status for mission complete ---
# #         'complete': {'text': 'TASKS COMPLETE', 'color': '#d1f7d3', 'icon': '🎉', 'class_suffix': 'on-track'}
# #         # --- END CHANGE 1 ---
# #     }
...
# #     time_per_step = total_expected_time / total_steps if total_steps > 0 else 0
# #     time_difference = (plan_index * time_per_step) - sim_time if plan_index > 0 else 0
# #     time_difference_int = int(abs(time_difference)) # Use int for comparison
    
# #     # --- NEW: Time Status Logic ---
# #     time_status_text = ""
# #     # Only report if absolute difference is 1 second or more
# #     if time_difference_int >= 1:
# #         time_status_text = f"Robot is {time_difference_int} seconds {'ahead' if time_difference > 0 else 'behind'} of schedule."
# #     # --- END NEW: Time Status Logic ---


# #     feature_alerts = []
    
# #     # --- NEW: Feature Status Change Logic (Suppress repetition) ---
...
# #     rmm_terrain = int(selected_robot_rmm_array.get('Bad_Terrain', 1))
# #     if hmm_terrain != rmm_terrain:
# #         feature_alerts.append("Encountering rough terrain (slowed movement)." if rmm_terrain == 0 else "Terrain has improved.")

# #     # Battery Status (New 0/1 scheme: 0 = dead/depleting, 1 = good)
# #     hmm_battery = int(selected_robot_hmm_array.get('Battery_status', 1)) if selected_robot_hmm_array else 1
# #     rmm_battery = int(selected_robot_rmm_array.get('Battery_status', 1))
# #     if hmm_battery != rmm_battery:
# #         if rmm_battery == 0: 
# #             feature_alerts.append("Battery status: Depleting/Dead.")
# #         elif rmm_battery == 1: 
# #             feature_alerts.append("Battery status: Good.")
...
# #     # --- END NEW: Feature Status Change Logic ---

# #     feature_text = " ".join(feature_alerts)

# #     status_key = 'stationary'
# #     details_msg = ""
# #     pkg_id = next((p['id'] for p in all_packages if p.get('carried_by') == robot_id), "Null")
# #     is_carrying = (pkg_id != "Null")
    
# #     # --- NEW: Quadrant and Position string template for readability ---
# #     pos_str = f"Position (X{x:.0f}, Y{y:.0f}) in the {quadrant} Quadrant."
...
# #     if robot_data.get('replan_flag') == True:
# #         status_key = 'replan'
# #         pkg_text = f" Carrying {pkg_id}." if is_carrying else ""
        
# #         # --- NEW: Replan message formatting ---
# #         details_msg = f"Route recalculated.{pkg_text} Current {pos_str} {combined_info}".strip()
        
# #     elif state == 'carrying':
# #         status_key = 'on_track'
        
# #         # --- NEW: Carry message formatting ---
# #         if is_carrying:
# #             plan = robot_data.get('plan', [])
# #             dest_x, dest_y = (plan[-1][0], plan[-1][1]) if plan else (x, y)
# #             dest_str = f"Position (X{dest_x:.0f}, Y{dest_y:.0f})"
            
# #             # Combine position and feature info smartly
# #             details_msg = f"Transporting **{pkg_id}** to {dest_str}. Currently at {pos_str}"
# #             if combined_info:
# #                  details_msg += f" {combined_info}"
# #             details_msg = details_msg.strip()
# #         else:
# #             # If state is 'carrying' but carried_by is Null, use moving message or default
# #             status_key = 'stationary' 
# #             details_msg = f"Robot status anomaly: supposed to be carrying but is not. Currently at {pos_str} {combined_info}".strip()

# #     elif state == 'moving':
# #         status_key = 'on_track'
# #         pkg_id_target = robot_data.get('target_package_id', "package")
# #         goal_x, goal_y = robot_data.get('immediate_goal_x', x), robot_data.get('immediate_goal_y', y)
        
# #         # --- NEW: Moving message formatting ---
# #         goal_str = f"Position (X{goal_x:.0f}, Y{goal_y:.0f})"
# #         details_msg = f"Moving to acquire **{pkg_id_target}** at {goal_str}. Currently at {pos_str}"
# #         if combined_info:
# #             details_msg += f" {combined_info}"
# #         details_msg = details_msg.strip()
...
# #     elif state == 'waiting':
# #         status_key = 'stationary'
# #         # --- NEW: Waiting message formatting ---
# #         details_msg = f"Robot is awaiting task. Currently at {pos_str} {combined_info}".strip()
        
# #     # --- CHANGE 1: Handle 'all_tasks_complete' state text ---
# #     elif state == 'all_tasks_complete':
# #         status_key = 'complete' 
# #         details_msg = f"Mission accomplished. All tasks complete. Robot is at {pos_str} {feature_text}".strip()
# #     # --- END CHANGE 1 ---
    
# #     else:
# #         status_key = 'stationary'
        
# #         # --- NEW: Unknown state message formatting ---
# #         details_msg = f"Robot in unknown state '{state}'. Currently at {pos_str} {combined_info}".strip()


# #     status_info = status_map.get(status_key, status_map['stationary'])
...
# #     details_content = html.Div([
# #         html.P(message_data['time_str'], style={'fontSize': '0.9em', 'color': '#555', 'margin': '10px 0 5px 0'}),
# #         # --- NOTE: Using dcc.Markdown here to render **bold** text in details_msg ---
# #         dcc.Markdown(message_data['details_msg'], style={'fontSize': '1.1em', 'margin': '5px 0 0 0'})
# #     ], style={'paddingLeft': '45px', 'paddingTop': '10px'})
...
# # # --- Study Screen Layouts (UNCHANGED) ---
# # def welcome_screen():
# #     return html.Div(style={'backgroundColor': '#1e1e1e', 'color': 'white', 'minHeight': '100vh',
# #                           'display': 'flex', 'flexDirection': 'column', 'alignItems': 'center',
# #                           'justifyContent': 'center', 'fontFamily': 'Arial'}, children=[
...
# # # --- MODIFIED: briefing_screen (UNCHANGED) ---
# # def briefing_screen(run_number, total_runs, scenario_num, view_type, framework_mode):
# #     view_name = "Simulator View (Map + Log)" if view_type == 'map' else "Textual Overview"
# #     framework_name = "WITH Communication Framework" if framework_mode == 'with_framework' else "WITHOUT Communication Framework"
...
# # # --- NEW: create_pause_question_screen (UNCHANGED) ---
# # def create_pause_question_screen(scenario_num, question_idx):
# #     """
# #     Generates the layout for a single pause-point question.
# #     question_idx is 0-based (0, 1, or 2).
...
# # # --- NEW: create_post_scenario_questionnaire (UNCHANGED) ---
# # def create_post_scenario_questionnaire(scenario_num, run_number, total_runs):
# #     """
# #     Generates the 6-question NASA-TLX-like questionnaire for the end of a run.
# #     """
...
# # # --- MODIFIED: post_study_questionnaire (UNCHANGED) ---
# # def post_study_questionnaire():
# #     questions = [
# #         {'text': 'How effective were the update messages in helping you understand the robots\' behavior?', 'type': 'scale', 'id': 'effectiveness', 'options': ['1 - Not at all effective', '2', '3', '4', '5 - Extremely effective']},
# #         {'text': 'How did this differ between demonstration types?', 'type': 'open', 'id': 'effectiveness_detail'},
...
# #     # --- NEW STORE FOR COMPREHENSIVE MESSAGE LOGGING ---
# #     dcc.Store(id='all-message-logs-store', data=[]),
# #     # --- NEW STORE FOR TRACKING D-PACKAGE DISCOVERIES (Change 3) ---
# #     dcc.Store(id='d-package-discovery-log', data={}),
...
# # # --- MODIFIED Clientside callback (to use queue and fast interval) (UNCHANGED) ---
# # app.clientside_callback(
# #     """
# #     function(n_intervals) {
# #         // --- NEW: Ensure queue exists on window ---
...
# # # --- MODIFIED Server callback (logs ONLY FIRST click AND updates open-messages-store) (UNCHANGED) ---
# # @app.callback(
# #     Output('interaction-log-store', 'data', allow_duplicate=True),
# #     Output('all-message-logs-store', 'data', allow_duplicate=True),
# #     Output('open-messages-store', 'data', allow_duplicate=True), # <-- ADDED OUTPUT
...
# # # --- MODIFIED: load_data_and_start_simulation (clears text stores and discovery log) ---
# # @app.callback(
# #     Output('scenario-data-store', 'data'),
# #     Output('hmm-data-store', 'data'),
# #     Output('current-frame-store', 'data', allow_duplicate=True),
...
# #     Output('open-messages-store', 'data', allow_duplicate=True),
# #     Output('message-timestamps-store', 'data', allow_duplicate=True),
# #     # --- Clear Text UI stores on load ---
# #     Output('robot-1-messages-store', 'data', allow_duplicate=True),
# #     Output('robot-2-messages-store', 'data', allow_duplicate=True),
# #     Output('robot-3-messages-store', 'data', allow_duplicate=True),
# #     Output('d-package-discovery-log', 'data', allow_duplicate=True), # <-- NEW OUTPUT
# #     Input('simulation-layout-loaded-signal', 'children'),
# #     State('study-state-store', 'data'),
# #     State('participant-store', 'data'),
# #     prevent_initial_call=True
...
# #     # Clear all message stores and discovery log for the new run
# #     return frames, hmms, start_frame, max_frames, max_frames, max_frames, marks, marks, marks, [], [], {}, [], [], [], {}

# # # --- MODIFIED: control_animation (controls both intervals) (UNCHANGED) ---
# # @app.callback(
# #     Output('animation-interval', 'disabled'),
# #     Output('click-logger-interval', 'disabled'), # --- ADDED ---
# #     Input('study-state-store', 'data'),
...
# # # --- MODIFIED: update_frame_and_check_pause (UNCHANGED) ---
# # @app.callback(
# #     Output('current-frame-store', 'data', allow_duplicate=True),
# #     Output('page-content', 'children', allow_duplicate=True),
# #     Output('study-state-store', 'data', allow_duplicate=True),
...
# #     State('message-timestamps-store', 'data'),
# #     State('all-message-logs-store', 'data'),
# #     State('d-package-discovery-log', 'data'), # <-- NEW STATE
# #     prevent_initial_call=True
# # )
# # def update_simulation_views(frame_idx, study_state,
# #                             scenario_data, hmm_data,
...
# #                             open_message_ids, all_messages_history,
# #                             message_timestamps, all_message_logs,
# #                             d_package_discovery_log):
# #     if not study_state or study_state.get('phase') != 'simulation':
# #         # --- MODIFIED: Return 15 no_updates (14 existing + 1 new store) ---
# #         return (no_update,) * 15
...
# #     new_message_timestamps = message_timestamps.copy() if message_timestamps else {}
# #     new_all_message_logs = all_message_logs.copy() if all_message_logs else []
# #     new_discovery_log = d_package_discovery_log.copy() # <-- NEW COPY
...
# #             new_message_timestamps,
# #             new_all_message_logs,
# #             new_discovery_log # <-- NEW RETURN VALUE
# #         )
...
# #     sim_time = current_frame_data.get('simulator time', 0)
# #     any_sync_occurred = False
# #     newly_generated_messages_for_feed = [] # Messages generated by robot sync
...
# #             rmm_keys_to_select = [
# #                 "state", "plan_index", "x", "y", "mission_time",
# #                 "Current_weather", "Bad_Terrain", "Battery_status", "Momentarily_offline", # <-- Added Bad_Terrain
# #                 "Replan_flag", "target_package_id", "plan", "immediate_goal_x", "immediate_goal_y",
...
# #     # --- CHANGE 3: Check for newly discovered D-packages ---
# #     newly_discovered_messages = []
    
# #     # 1. Iterate through all packages in the current frame
# #     for p in packages:
# #         pkg_id = p.get('id', '')
        
# #         # Check if it's a 'd' package that is marked as discovered (1)
# #         is_d_package_and_discovered = pkg_id.startswith('d') and p.get('Discovered', 0) == 1
        
# #         # Only process if:
# #         # a) it's a discovered 'd' package
# #         # b) we haven't logged a discovery message for it yet
# #         # c) the discovery was actualy performed by a robot (Discovered_by is not Null)
# #         if is_d_package_and_discovered and pkg_id not in new_discovery_log and p.get('Discovered_by', 'Null') != 'Null':
            
# #             # --- Smart Communication Logic ---
# #             discovered_by = p.get('Discovered_by', 'System')
# #             assigned_to = p.get('Assigned_to', 'Null')
            
# #             # Message structure based on assignment
# #             message_text = f"Package **{pkg_id}** discovered by **{discovered_by.title()}**."
            
# #             if assigned_to == discovered_by:
# #                 # Discovered by R1, Assigned to R1 -> R1 is after it
# #                 message_text = f"Package **{pkg_id}** discovered by **{discovered_by.title()}** and added to its own manifest. **{discovered_by.title()}** is proceeding with pickup."
# #             elif assigned_to != 'Null':
# #                 # Discovered by R1, Assigned to R2 -> R1 found it, R2 is taking over
# #                 message_text = f"Package **{pkg_id}** discovered by **{discovered_by.title()}**. Pickup assigned to **{assigned_to.title()}**."
# #             else:
# #                 # Discovered by R1, Assigned to Null -> It's found but no one is assigned yet
# #                 message_text += " It is awaiting assignment."
                
# #             # --- End Smart Communication Logic ---
            
# #             # 2. Log the discovery message to the all-message-logs
# #             # We use the Discoverer's ID for the log entry's robot_id, but the System as title for the UI
# #             msg_id = f"pkg_discovery-{pkg_id}-{sim_time:.2f}"
# #             appear_time = time.time()
            
# #             log_entry = create_message_log_entry(
# #                 message_id=msg_id,
# #                 robot_id=discovered_by,
# #                 scenario_num=scenario_num,
# #                 condition_idx=condition_idx,
# #                 frame_idx=frame_idx,
# #                 sim_time=sim_time,
# #                 message_type='package_discovery',
# #                 message_text=message_text,
# #                 participant_id=participant_data.get('id'),
# #                 appear_time=appear_time,
# #                 robot_state='discovery_event',
# #                 robot_x=p.get('x'),
# #                 robot_y=p.get('y')
# #             )
# #             new_all_message_logs.append(log_entry)

# #             # 3. Create message data for rendering 
# #             discovery_message_data = {
# #                 'message_id': msg_id,
# #                 'robot_id_title': 'System', # System status for discovery events
# #                 'status_icon': '✨',
# #                 'status_text': 'NEW DISCOVERY',
# #                 'status_class_suffix': 'discovery', # Use new class
# #                 'time_str': datetime.fromtimestamp(appear_time).strftime('%I:%M:%S %p'),
# #                 'details_msg': message_text
# #             }
            
# #             # 4. Create the component for the Map UI feed
# #             newly_discovered_messages.append(render_message_component(
# #                 discovery_message_data,
# #                 current_open_message_ids,
# #                 is_new=True
# #             ))
            
# #             # 5. Update the log to prevent future messages for this package
# #             new_discovery_log[pkg_id] = sim_time
            
# #     # Combine messages from robot syncs and new package discoveries
# #     updated_all_messages = newly_generated_messages_for_feed + newly_discovered_messages + (all_messages_history or [])
...
# # # --- *** NEW/MODIFIED: Callbacks to link stores to Text UI (UNCHANGED) ---
# # @app.callback(
# #     Output('robot-1-messages', 'children'),
# #     Input('robot-1-messages-store', 'data'),
# #     State('open-messages-store', 'data') # <-- ADD STATE
...
# # # --- NEW: submit_pause_question (UNCHANGED) ---
# # @app.callback(
# #     Output('page-content', 'children', allow_duplicate=True),
# #     Output('responses-store', 'data', allow_duplicate=True),
# #     Output('study-state-store', 'data', allow_duplicate=True),
...
# # # --- NEW: submit_post_scenario_questions (UNCHANGED) ---
# # @app.callback(
# #     Output('page-content', 'children', allow_duplicate=True),
# #     Output('responses-store', 'data', allow_duplicate=True),
# #     Output('study-state-store', 'data', allow_duplicate=True),
...
# # # --- MODIFIED: submit_final_questionnaire (UNCHANGED) ---
# # @app.callback(
# #     Output('page-content', 'children', allow_duplicate=True),
# #     Input('submit-final-btn', 'n_clicks'),
# #     State('participant-store', 'data'),
...
# #     # --- MODIFIED: Added style for 'discovery' messages ---
# #     with open("assets/style.css", "w") as f:
# #         f.write("""
# # body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; }
# # .robot-column { display: flex; flex-direction: column; gap: 15px; flex: 1; height: 100%; }
...
# # .message-container-details.on-track {
# #     background-color: #e0f5e1; /* Green for on-track/complete */
# #     border-color: #b0dcb0;
# # }
...
# # /* NEW: Style for Package Discovery messages */
# # .message-container-details.discovery {
# #     background-color: #e0f7fa; /* Light blue/cyan for discovery */
# #     border-color: #b2ebf2;
# # }
...
# #     # Use debug=False for actual study deployment
# #     app.run(debug=False, host='0.0.0.0', port=9231)
...
#     if robot_data.get('replan_flag') == True:
#         status_key = 'replan'
#         # --- MODIFICATION: Prevent 'Carrying N/A' ---
#         pkg_id_text = ""
#         if state == 'carrying':
#             pkg_id = next((p['id'] for p in all_packages if p.get('carried_by') == robot_id), None) # <-- Changed to None
#             if pkg_id: # <-- Added check
#                 pkg_id_text = f" Carrying {pkg_id}."
#         # --- MODIFICATION: Improved formatting ---
#         details_msg = f"Route recalculated.{pkg_id_text} Current Position is (X{x:.0f}, Y{y:.0f}) in the {quadrant} Quadrant. {combined_info}".strip()
#     elif state == 'carrying':
#         status_key = 'on_track'
#         pkg_id = next((p['id'] for p in all_packages if p.get('carried_by') == robot_id), "N/A")
#         plan = robot_data.get('plan', [])
#         dest_x, dest_y = (plan[-1][0], plan[-1][1]) if plan else (x, y)
#         # --- MODIFICATION: Improved formatting ---
#         details_msg = f"Transporting {pkg_id} to Position (X{dest_x:.0f}, Y{dest_y:.0f}). Currently at Position (X{x:.0f}, Y{y:.0f}) in the {quadrant} Quadrant. {feature_text}".strip()
#     elif state == 'moving':
#         status_key = 'on_track'
#         pkg_id = robot_data.get('target_package_id', "package")
#         goal_x, goal_y = robot_data.get('immediate_goal_x', x), robot_data.get('immediate_goal_y', y)
#         # --- MODIFICATION: Improved formatting ---
#         details_msg = f"Moving to acquire {pkg_id} at Position (X{goal_x:.0f}, Y{goal_y:.0f}). Currently at Position (X{x:.0f}, Y{y:.0f}) in the {quadrant} Quadrant. {feature_text}".strip()
#     elif state == 'waiting':
#         status_key = 'stationary'
#         # --- MODIFICATION: Improved formatting ---
#         details_msg = f"Robot is awaiting task at Position (X{x:.0f}, Y{y:.0f}) in the {quadrant} Quadrant. {combined_info}".strip()
#     elif state == 'all_tasks_complete':
#         status_key = 'on_track' # Use 'on_track' style (green)
#         # --- MODIFICATION: Improved formatting ---
#         details_msg = f"All tasks complete. Robot is at Position (X{x:.0f}, Y{y:.0f}) in the {quadrant} Quadrant. {feature_text}".strip()
#     else:
#         status_key = 'stationary'
#         # --- MODIFICATION: Improved formatting ---
#         details_msg = f"Robot in unknown state '{state}' at Position (X{x:.0f}, Y{y:.0f}) in the {quadrant} Quadrant. {combined_info}".strip()
#     # --- END QUADRANT ADDITION ---
...
#     # Use debug=False for actual study deployment
#     app.run(debug=False, host='0.0.0.0', port=9249)
...
""")
    ...
    # Use debug=False for actual study deployment
    app.run(debug=False, host='0.0.0.0', port=9279)