...
# # --- REMOVED LATIN_SQUARE ---

# # --- ADDED 24-COMBINATION STUDY DESIGN ---
# # Scenario Mapping:
# # e1a = 1, e1b = 2, e2a = 3, e2b = 4, e3a = 5, e3b = 6
# #
# # Condition Mapping:
# # 0 = Map + Framework
# # 1 = Map + No Framework
# # 2 = Textual + Framework
# # 3 = Textual + No Framework
# STUDY_DESIGN_TABLE = [
...
# # --- REMOVED assign_participant_scenarios ---
...
# # create_figure_for_frame is UNCHANGED from test_study.py
...
# # --- *** REPLACED create_rich_status_message *** ---
...
#         id={'type': 'message-summary', 'id': message_id},  # <-- MODIFIED
#         n_clicks=0  # <-- ADDED
#     )
...
# # welcome_screen is UNCHANGED from test_study.py
# def welcome_screen():
...
# # question_screen is UNCHANGED from test_study.py
...
# # post_study_questionnaire is UNCHANGED from test_study.py
...
# # create_simulation_layout is UNCHANGED from test_study.py
...
# # thank_you_screen is UNCHANGED from test_study.py
...
# # --- App Layout ---
# # The layout definition is UNCHANGED from test_study.py
# app.layout = html.Div([
...
#     # Store the 24-track and participant info
#     participant_data = {
#         'id': participant_id,
#         'name': name,
#         'track': assigned_track, # <-- MODIFIED: Store the full track
#         'start_time': datetime.now().isoformat()
#     }

#     # Set up the new state based on runs, not scenarios/conditions
#     study_state = {
#         'current_run_idx': 0, # <-- NEW STATE
#         'current_question_idx': 0,
#         'phase': 'briefing',
#         'segment_start_time': None
#     }

#     # Get info for the *first* run
...
#     # Call briefing screen with new arguments
...
# # begin_scenario is UNCHANGED from test_study.py
# @app.callback(
...
# # --- MODIFIED load_data_and_start_simulation ---
# @app.callback(
...
#     # Get scenario number from the *current run*
...
# # control_animation is UNCHANGED from test_study.py
# @app.callback(
...
# # --- MODIFIED programmatically_switch_view ---
# @app.callback(
...
#     # Use the helper function to get names
#     view_type, framework_mode = get_condition_names(condition_idx)
#     view_name = "Simulator View (Map + Log)" if view_type == 'map' else "Textual Overview"
#     framework_name = "WITH Communication Framework" if framework_mode == 'with_framework' else "WITHOUT Communication Framework"
...
# # --- *** MODIFIED update_simulation_views (from 1st fix) *** ---
# # This callback is intentionally separate from click logging.
# # It now correctly reads the 'open_message_ids' state to render.
# @app.callback(
...
#     Input('study-state-store', 'data'), 
#     Input('open-messages-store', 'data'), # <-- LISTEN for state changes
#     State('scenario-data-store', 'data'),
...
#     prevent_initial_call=True
# )
# def update_simulation_views(frame_idx, study_state, open_message_ids, # <-- ADDED
#                             scenario_data, hmm_data,
#                             participant_data, hist1, hist2, hist3,
#                             all_messages_history,
#                             message_timestamps):

#     # Only update on frame or study_state change, not just open_message_ids
#     triggered_id = callback_context.triggered_id
#     if triggered_id == 'open-messages-store':
#         # We need to *re-render* the messages, but not re-run logic
#         pass
    
#     if not study_state or study_state.get('phase') != 'simulation':
#         # Return 12 no_updates, matching the 12 Outputs
#         return (no_update,) * 12 

#     # --- Get current run info *once* for logging ---
#     current_run_idx = study_state['current_run_idx']
#     current_run_info = participant_data['track'][current_run_idx]
#     scenario_num = current_run_info[0]
#     condition_idx = current_run_info[1]
...
#     # Use the run info fetched at the top
...
#     # Rebuild history lists to respect the 'open_message_ids' state
#     new_robot_message_outputs = []
#     for hist in histories:
#         rebuilt_hist = []
#         if not hist:
#             new_robot_message_outputs.append([])
#             continue
        
#         hist_list = hist if isinstance(hist, list) else [hist]
        
#         # We check the component *type* to avoid trying to process dividers
#         for msg_component in hist_list:
#             if (isinstance(msg_component, dict) and 
#                 msg_component.get('type') == 'Details' and 
#                 'props' in msg_component and 
#                 isinstance(msg_component['props'].get('id'), dict)):
                
#                 msg_id = msg_component['props']['id']['id']
#                 # Create a *new* component dict to avoid modifying state
#                 new_msg_component = msg_component.copy()
#                 new_msg_component['props'] = new_msg_component['props'].copy()
#                 new_msg_component['props']['open'] = (msg_id in open_message_ids)
#                 rebuilt_hist.append(new_msg_component)
#             else:
#                 # Append dividers, etc.
#                 rebuilt_hist.append(msg_component)
#         new_robot_message_outputs.append(rebuilt_hist)
...
#     # This logic only runs if the timer ticked, not just a click
#     if triggered_id != 'open-messages-store':
#         for i in range(1, 4):
#             robot_id = f'robot{i}'
...
#                             sim_time,
#                             packages,
#                             open_message_ids, # Pass the state list
...
#                         # Prepend the new message to the *rebuilt* history
#                         updated_history = [new_message_div]
#                         current_hist = new_robot_message_outputs[i-1] 
...
#     # Rebuild the main feed history as well
#     rebuilt_all_messages_history = []
#     if all_messages_history:
#         for msg_component in all_messages_history:
#             if (isinstance(msg_component, dict) and 
#                 msg_component.get('type') == 'Details' and 
#                 'props' in msg_component and 
#                 isinstance(msg_component['props'].get('id'), dict)):
...
#                 msg_id = msg_component['props']['id']['id']
#                 # Create a *new* component dict
#                 new_msg_component = msg_component.copy()
#                 new_msg_component['props'] = new_msg_component['props'].copy()
#                 new_msg_component['props']['open'] = (msg_id in open_message_ids)
#                 rebuilt_all_messages_history.append(new_msg_component)
#             else:
#                 rebuilt_all_messages_history.append(msg_component)

#     updated_all_messages = newly_generated_messages_for_feed + rebuilt_all_messages_history
...
# # update_snapshot is UNCHANGED from test_study.py
# @app.callback(
...
# # --- MODIFIED log_graph_click (to fix logging context) ---
# @app.callback(
...
#         'scenario': scenario_num, # <-- Fixed context
#         'condition': condition_idx, # <-- Fixed context
#         'frame': frame_idx,
#         'type': 'graph_click',
...
# # --- *** REPLACED log_message_click CALLBACK *** ---
# @app.callback(
#     Output('interaction-log-store', 'data', allow_duplicate=True),
#     Output('open-messages-store', 'data', allow_duplicate=True),
#     Input({'type': 'message-summary', 'id': ALL}, 'n_clicks'), # <-- MODIFIED INPUT
#     State('interaction-log-store', 'data'),
#     State('participant-store', 'data'),
#     State('study-state-store', 'data'),
#     State('current-frame-store', 'data'),
...
#     prevent_initial_call=True
# )
# def log_message_click(message_n_clicks, # <-- RENAMED
#                       interactions, participant_data, study_state, frame_idx,
#                       open_message_ids, message_timestamps):
    
#     if not callback_context.triggered or not participant_data or not study_state:
#         raise PreventUpdate

#     # Check if n_clicks is None or 0, which happens on load
#     triggered_item = callback_context.triggered[0]
...
#         if component_id_dict.get('type') != 'message-summary': # <-- CHECK TYPE
#             raise PreventUpdate
...
#         # --- NEW TOGGLE LOGIC ---
#         action = ""
#         if message_id in new_open_message_ids:
...
#         appear_time = message_timestamps.get(message_id)
...
#         print(f"Logging message click: {action} on {message_id}")
#         new_interactions.append(interaction_entry)
            
#         return new_interactions, list(new_open_message_ids)

#     except Exception as e:
#         print(f"Error parsing message click: {e}, prop_id: {prop_id}")
#         raise PreventUpdate
# # --- *** END REPLACEMENT *** ---


# # --- REPLACED submit_questions CALLBACK ---
# @app.callback(
#     Output('page-content', 'children', allow_duplicate=True),
...
#     prevent_initial_call=True
# )
# def submit_questions(n_clicks, study_state, participant_data, responses, q_values, q_ids, interactions):
#     if n_clicks == 0 or not study_state or not participant_data:
#         raise PreventUpdate
...
#         'run_number': current_run_idx + 1, # <-- New field
#         'scenario': scenario_num,
#         'condition': condition_idx,
#         'view_type': view_type,
...
#             return briefing_screen(next_run_idx + 1, total_runs, next_scenario_num, next_view_type, next_framework), responses, new_study_state
#         else:
#             # Finished all runs
...
# # submit_final_questionnaire is UNCHANGED from test_study.py
# @app.callback(
#     Output('page-content', 'children', allow_duplicate=True),
...
#         os.makedirs("assets")
#     # The CSS block is UNCHANGED from test_study.py
...
# }
# .divider { text-align: center; color: #777; font-size: 0.9em; margin: 15px 0; }
...
#     border-color: #a4a450;
# }
# .message-box-dark-theme .message-container-details p {
...
# create_rich_status_message is UNCHANGED from test_study.py
def create_rich_status_message(robot_data, sim_time, all_packages, open_message_ids, selected_robot_hmm_array, selected_robot_rmm_array, scenario_id):
    if open_message_ids is None:
        open_message_ids = []
...
# --- MODIFIED update_simulation_views (Added one print statement for logging) ---
@app.callback(
    Output('simulation-graph', 'figure'),
    Output('robot-1-messages', 'children'),
    Output('robot-2-messages', 'children'),
    ...
    Output('message-timestamps-store', 'data'), 
    Input('current-frame-store', 'data'),
    Input({'type': 'status-message', 'id': ALL}, 'open'), 
    Input('study-state-store', 'data'), 
    State('scenario-data-store', 'data'),
    State('hmm-data-store', 'data'),
    State('participant-store', 'data'),
...
def update_simulation_views(frame_idx, message_opens, study_state,
                            scenario_data, hmm_data,
                            participant_data, hist1, hist2, hist3,
            ...
            if '.open' in prop_id:
                try:
                    ...
                    if component_id_dict.get('type') == 'status-message':
                        ...
                        # Use the run info defined above
                        interaction_entry = {
                            'timestamp_iso': datetime.now().isoformat(),
                            'timestamp_unix': click_time,
                            'participant': participant_data.get('id'),
                        ...
                        # --- THIS IS THE NEW LINE FOR VISIBLE LOGGING ---
                        print(f"LOGGED MESSAGE CLICK: {message_id}, Action: {action}, Time-to-Click: {time_to_click:.2f}s")
                        # -------------------------------------------------
    ...
    open_message_ids = list(new_open_message_ids) 
...
# --- MODIFIED log_graph_click (Added one print statement for logging) ---
@app.callback(
    Output('interaction-log-store', 'data', allow_duplicate=True),
    Input('simulation-graph', 'clickData'),
    State('interaction-log-store', 'data'),
...
def log_graph_click(clickData, interactions, participant_data, study_state, frame_idx):
    ...
    # --- THIS IS THE NEW LINE FOR VISIBLE LOGGING ---
    print(f"LOGGED GRAPH CLICK: {details}")
    # -------------------------------------------------
...
/* This is for the dark-theme log on the map view */
.message-box-dark-theme .message-container-details.replan {
...
.message-box-dark-theme .message-container-details summary {
    color: #eee;
}
.message-box-dark-theme .message-container-details p {
    color: #ccc;
