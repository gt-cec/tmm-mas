...
app.layout = html.Div([
    ...
    # NEW: Hidden div to capture message clicks
...
# --- NEW: Clientside callback to capture message clicks ---
app.clientside_callback(
    """
    function(n_intervals) {
        // Attach click listeners to all details elements
        const detailsElements = document.querySelectorAll('details[id*="status-message"]');
        detailsElements.forEach(function(details) {
            details.addEventListener('toggle', function(event) {
                const messageId = details.id;
                ...
                // Store in a global variable for the server callback to pick up
                window.lastMessageClick = clickData;
            });
        });
        return window.lastMessageClick || null;
    }
    """,
...
# --- NEW: Server callback to log message clicks ---
@app.callback(
...
def log_message_clicks_from_clientside(click_data, interactions, participant_data, study_state, frame_idx, message_timestamps):
    if not click_data or not all([participant_data, study_state]):
        raise PreventUpdate
    
    new_interactions = interactions.copy() if interactions is not None else []
    
    try:
        click_info = json.loads(click_data) if isinstance(click_data, str) else click_data
        message_id = click_info.get('messageId', '')
        is_open = click_info.get('isOpen', False)
        click_time = click_info.get('timestamp', time.time()) / 1000.0  # Convert ms to seconds
        
        # Extract message ID from the full ID string
        if 'status-message' in message_id:
            # Parse the pattern: {"id":"robot1-123.45","type":"status-message"}
            parts = message_id.split('"id":"')
            if len(parts) > 1:
        ...
        print(f"Logged message click: {message_id} - {action}")
...
# --- MODIFIED update_simulation_views (REMOVED message click logging) ---
@app.callback(
...
""")
    ...
    app.run(debug=False, host='0.0.0.0', port=9927)
