# This study script resets the messages after every set of questions. testing in progress.
# Please remove white space, maybe add flash/animation to new message, remove simulation log and bring messages to full right view, ty!!
...
def update_simulation_views(frame_idx, message_opens, study_state, # Added study_state
    ...
    # Store previous message states to detect changes
    previous_open_states = {msg_id: (msg_id in open_message_ids) for msg_id in new_message_timestamps.keys()}

    # Handle message click interactions by comparing states
    if message_opens:  # message_opens is a list of True/False values
        # Get all message IDs from the callback context
        ctx_inputs_list = callback_context.inputs_list
        
        # Find the status-message inputs
        for input_group in ctx_inputs_list:
            if isinstance(input_group, list):
                for idx, item in enumerate(input_group):
                    if idx < len(message_opens):
                        # item is the Input object, extract the id
                        try:
                            component_id = ctx_inputs_list[1][idx]  # Second input group is status-message
                            if isinstance(component_id, dict) and component_id.get('type') == 'status-message':
                                message_id = component_id['id']
                                current_state = message_opens[idx]
                                previous_state = previous_open_states.get(message_id, False)
                                
                                # Detect if state changed
                                if current_state != previous_state:
                                    action = "opened" if current_state else "closed"
                            ...
                            print(f"Error in message click detection: {e}")
                            import traceback
                            traceback.print_exc()

    # Also update open_message_ids based on current message_opens state
    if message_opens:
        # Sync the open messages based on current state
        try:
            ctx_inputs_list = callback_context.inputs_list
            if len(ctx_inputs_list) > 1 and isinstance(ctx_inputs_list[1], list):
                for idx, is_open in enumerate(message_opens):
                    if idx < len(ctx_inputs_list[1]):
                        component_id = ctx_inputs_list[1][idx]
                        if isinstance(component_id, dict) and component_id.get('type') == 'status-message':
                            message_id = component_id['id']
                            if is_open:
                                new_open_message_ids.add(message_id)
            ...
            print(f"Error syncing message states: {e}")
...
/* --- END OF NEW CSS --- */
    ...
    app.run(debug=False, host='0.0.0.0', port=1035)