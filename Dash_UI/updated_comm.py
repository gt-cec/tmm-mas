...
def create_rich_status_message(robot_data, sim_time, all_packages, open_message_ids, selected_robot_hmm_array, selected_robot_rmm_array, scenario_id):
    """
    Creates rich, collapsible status messages for all robot states.
    """
    ...
    # --- Combine Time and Feature Alerts ---
    time_and_features = []
    if time_status_text:
    ...
    if robot_data.get('replan_flag') == True:
        status_key = 'replan'
        if state == 'moving':
            base_msg = f"Route recalculated. Heading to Position (X{x:.0f}, Y{y:.0f})."
        elif state == 'waiting':
            base_msg = f"Route recalculated. Waiting at Position (X{x:.0f}, Y{y:.0f})."
        elif state == 'carrying':
            pkg_id = next((p['id'] for p in all_packages if p.get('carried_by') == robot_id), "N/A")
            base_msg = f"Route recalculated. Carrying {pkg_id} to Position (X{x:.0f}, Y{y:.0f})."
        else:
            base_msg = f"Route recalculated. Status: {state} at Position (X{x:.0f}, Y{y:.0f})."
        ...
        if plan and len(plan) > 0:
            ...
            base_msg = f"Transporting {pkg_id} to Position (X{dest_x:.0f}, Y{dest_y:.0f}). Currently at Position (X{x:.0f}, Y{y:.0f})."
        else:
            base_msg = f"Transporting {pkg_id}. Currently at Position (X{x:.0f}, Y{y:.0f})."
        ...
        base_msg = f"Moving to acquire {pkg_id} at Position (X{goal_x:.0f}, Y{goal_y:.0f})."

    elif state == 'waiting':
        status_key = 'stationary'
        base_msg = f"Robot is awaiting task at Position (X{x:.0f}, Y{y:.0f})."
    
    else:
        status_key = 'stationary'
        base_msg = f"Robot in unknown state '{state}' at Position (X{x:.0f}, Y{y:.0f})."

    # Universally combine base message with combined_info
    details_msg = f"{base_msg} {combined_info}".strip()
    
    # --- Build Universal Collapsible Component ---
    ...
    return html.Details([summary, details_content],
                        className='message-container-details',  # Use new generic class
                        id={'type': 'status-message', 'id': message_id}, # Use new generic type
                        open=(message_id in open_message_ids),
                        style={'backgroundColor': status_info['color']} # Apply color inline
                       )
...
@app.callback(
    ...
    Input({'type': 'status-message', 'id': ALL}, 'open'), # MODIFIED: Changed type
    State('open-messages-store', 'data'),
    prevent_initial_call=True
)
def update_open_messages_state(open_states, current_open_ids):
    """
    This callback updates the list of which status messages are currently open.
    """
    triggered_id = callback_context.triggered_id
...
/* MODIFIED: Replaced .replan-message-container with .message-container-details */
.message-container-details {
    /* background-color is now set inline via style prop */
    padding: 15px;
    border-radius: 5px;
    margin-bottom: 13px;
