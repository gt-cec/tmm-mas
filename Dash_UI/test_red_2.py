...
from dash.dependencies import ALL # <-- IMPORT ALL
...
THRESHOLD_VALUES = {
    'with_framework': {
        1: 12,  # Scenario 1, with framework
        2: 12,  # Scenario 2, with framework
        3: 15,
        4: 9
    },
    'without_framework': {
        1: 1,  # Scenario 1, without framework
        2: 1,  # Scenario 2, without framework
        3: 1,
        4: 1
    }
}
...
def get_quadrant(x, y, grid_width=20, grid_height=20):
...
def weather_str(w):
    return "good" if int(w) == 1 else "bad"


def create_rich_status_message(robot_data, sim_time, all_packages, open_message_ids=None):
    ...
    weather_val = robot_data.get('Current_weather', 1)
    weather = weather_str(weather_val)
    status_map = {
    ...
    # Check for replan flag FIRST.
    if robot_data.get('replan_flag') == True:
        message_id = f"{robot_id}-{sim_time}"
        state = robot_data.get('state')
        if state == 'moving':
            details_msg = f"Route recalculated. Heading towards the {quadrant} quadrant. Current weather is {weather}."
        elif state == 'waiting':
            details_msg = f"Route recalculated. Waiting at the {quadrant} quadrant. Current weather is {weather}."
        elif state == 'carrying':
            details_msg = f"Route recalculated. Carrying package towards the {quadrant} quadrant. Current weather is {weather}."
        else:
            details_msg = f"Route recalculated. Status: {state} at the{quadrant} quadrant. Current weather is {weather}."
    ...
    status_key, details_text, state = '', '', robot_data.get('state')
    if state == 'carrying':
        status_key = 'on_track'
        pkg_id = next((p['id'] for p in all_packages if p.get('carried_by') == robot_id), "N/A")
        details_text = f"Transporting package {pkg_id} towards {quadrant} quadrant. Current weather is {weather}."
    elif state == 'moving':
        status_key = 'on_track'
        pkg_id = robot_data.get('target_package_id', "package")
        details_text = f"Moving to acquire package {pkg_id} in the {quadrant} quadrant. Current weather is {weather}."
    elif state == 'waiting':
        status_key = 'stationary'
        details_text = f"Robot is recharging, awaiting package in the {quadrant} quadrant. Current weather is {weather}."
    return html.Div([
        ...
        'backgroundColor': status_map[status_key]['color'], 'padding': '15px', 'borderRadius': '5px',
        'marginBottom': '13px', 'border': '1px solid #ddd', 'color': '#212529'
    })



# # --- NEW/UPDATED ---
# def create_rich_status_message(robot_info, sim_time, packages, open_message_ids, selected_robot_hmm_array,selected_robot_rmm_array )
#     if open_message_ids is None:
#         open_message_ids = [] # Ensure it's always a list

#     time_str = datetime.now().strftime('%I:%M:%S %p')
#     robot_id = robot_data.get('id', 'N/A')
#     pos = f"({robot_data.get('x', 0):.1f}, {robot_data.get('y', 0):.1f})"
#     status_map = {
#         'replan': {'text': 'REPLANNING', 'color': '#ffdede', 'icon': '🔄'},
#         'on_track': {'text': 'ON TRACK', 'color': '#e0f5e1', 'icon': '✅'},
#         'stationary': {'text': 'STATIONARY', 'color': '#fff9c4', 'icon': '⏸️'}
...
#     # Check for replan flag FIRST. This has the highest priority.
#     if robot_data.get('replan_flag') == True:
#         # Create a unique ID for this specific replan event
#         message_id = f"{robot_id}-{sim_time}"
...
#         details_content = html.Div([
#              html.P(time_str, style={'fontSize': '0.9em', 'color': '#555', 'margin': '10px 0 5px 0'}),
#              html.P(f"Route recalculated. Current position is {pos}.", style={'fontSize': '1.1em', 'margin': '5px 0 0 0'})
#         ], style={'paddingLeft': '45px', 'paddingTop': '10px'})

#         # Return an html.Details with a pattern-matching ID and the 'open' property set dynamically
#         return html.Details([summary, details_content], 
#                             className='replan-message-container', 
#                             id={'type': 'replan-message', 'id': message_id},
#                             open=(message_id in open_message_ids))
...
#         status_key = 'on_track'
#         pkg_id = next((p['id'] for p in all_packages if p.get('carried_by') == robot_id), "N/A")
#         details_text = f"Transporting {pkg_id}. Position: {pos}."
#     elif state == 'moving':
#         status_key = 'on_track'
#         pkg_id = robot_data.get('target_package_id', "target")
#         details_text = f"Moving to acquire {pkg_id}. Position: {pos}."
#     else:  # Covers 'waiting' and any other default state
#         status_key = 'stationary'
#         details_text = f"Awaiting task at position {pos}."
...
app.layout = html.Div(style={'backgroundColor': '#1e1e1e', 'color': 'white', 'fontFamily': 'Arial', 'height': '100vh', 'display': 'flex', 'flexDirection': 'column'}, children=[
    ...
    # --- NEW/UPDATED ---
    dcc.Store(id='open-messages-store', data=[]),  # <-- THIS STORES THE STATE
...
@app.callback(
    ...
    prevent_initial_call='initial_duplicate'  # CHANGE THIS
...
def auto_pause_at_end(current_frame, scenario_data):
    ...
    if current_frame >= max_frames:
        return False  # Set is_playing to False to pause the animation
    return no_update
...
def update_current_frame(n_intervals, n_restarts, is_playing, current_frame, scenario_data):
    ...
    if is_playing and ctx == 'animation-interval':
        # next_frame = (current_frame + 1) % (max_frames + 1)
        if current_frame < max_frames:
            next_frame = current_frame + 1
        else:
            next_frame = max_frames # Stay on the last frame
...
def update_open_messages_state(open_states, current_open_ids):
    ...
    This version is more robust and avoids JSON parsing errors.
    """
    # Use triggered_id, which Dash provides as a dictionary for pattern-matching IDs
    triggered_id = callback_context.triggered_id
    
    # Check if the trigger came from one of our pattern-matched components
    if not isinstance(triggered_id, dict):
        return no_update # Ignore triggers from non-dictionary IDs

    # Get the value (True if open, False if closed) from the full triggered context
    ...
    if is_open:
        ...
        open_ids_set.discard(message_id) # discard won't error if it's not there
...
# def update_all_outputs(frame_idx, scenario_id, scenario_data, hmm_data, framework_mode, hist1, hist2, hist3, open_message_ids): # <-- ADDED PARAMETER
...
#     log_text += "🤖 ROBOTS:\n" + "-"*20 + "\n"
#     if not robots_dict: log_text += "(No robot data)\n"
#     else:
...
#     log_text += "\n📦 PACKAGES:\n" + "-"*20 + "\n"
#     if not packages: log_text += "(No package data)\n"
#     else:
#         for p in packages:
...
#             if raw_robot_data.get('replan_flag') == True:
#                 is_replan_event = True
                
                
#             rmm_keys_to_select = [
#                 "state", "plan_index", "x", "y", "mission_time",
#                 "Current_weather", "Battery_status", "Momentarily_offline",
#                 "Replan_flag", "target_package_id"
#             ]
...
#                 updated_hmm_array, sync_event_happened = dynamic_deviation_threshold_multi_logic( hmm_array=selected_robot_hmm_array,
...
#             # Backend logic would go here, for now it's commented out
#             # selected_robot_rmm_array, selected_robot_hmm_array = ...
#             # if selected_robot_hmm_array:
...
#             # --- UPDATED: Pass the list of open IDs ---
#             new_message_div = create_rich_status_message(robot_info, sim_time, packages, open_message_ids)
...
#     new_interval = 2000 if is_replan_event else UPDATE_INTERVAL_MS
...
def update_all_outputs(frame_idx, scenario_id, scenario_data, hmm_data, framework_mode, hist1, hist2, hist3, open_message_ids):
    ...
    new_messages = []
    sim_time = current_frame_data.get('simulator time', 0)
    any_sync_occurred = False  # Track if ANY robot had a sync
    ...
    for i in range(1, 4):
        ...
        if robot_id in robots_dict:
            ...
            if selected_robot_hmm_array:
                ...
                # ONLY create message if sync occurred
                if sync_occurred:
                    ...
                    # No sync - keep existing history unchanged
                    new_messages.append(histories[i-1])
            else:
                new_messages.append(histories[i-1])
        else:
    ...
    # Set slower interval only if ANY sync occurred
...
/* Progress Bar Slider in Text View */
...
/* --- NEW CSS FOR THE RED REPLAN BUTTON --- */
.replan-message-container {
    background-color: #ffdede; /* Red background for the whole box */
    padding: 15px;
    border-radius: 5px;
    margin-bottom: 13px;
...
/* Style the summary line to make it look like a header/button */
.replan-message-container summary {
    cursor: pointer;
    outline: none; /* Removes the default outline on click */
    display: block; /* Ensures it takes up the full width */
}
/* Hides the default dropdown arrow icon from the browser */
