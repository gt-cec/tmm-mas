...
def create_rich_status_message_data(robot_data, sim_time, all_packages, selected_robot_hmm_array, selected_robot_rmm_array, scenario_id):
    ...
    # --- MODIFICATION: Mission Time/ETA Calculation (Updated) ---
    plan_index = selected_robot_rmm_array.get('plan_index', 0)
    # mission_time = selected_robot_rmm_array.get('mission_time', 0) # REMOVED: Mission time logging
    ...
    # NEW MODIFICATION: Ensure ETA is always positive
    eta_seconds = abs(int(remaining_steps * time_per_step))
    
    # NEW MODIFICATION: Only include ETA in the time status text
    time_status_text = f"Estimated Time to Goal: {eta_seconds} seconds."
    # --- END MODIFICATION: Mission Time/ETA Calculation ---
    ...
    # --- MODIFICATION: ADD BAD TERRAIN CHECK (and collect icon data) ---
    ...
    # --- MODIFICATION: UPDATE BATTERY STATUS LOGIC ---
    ...
    # --- MODIFICATION: Move ETA to top of list ---
    details_msg = [time_status_text]
    # --- END MODIFICATION: Move ETA to top of list ---
    ...
    if robot_data.get('replan_flag') == True:
        ...
        details_msg.extend([ # Append after ETA
...
def update_simulation_views(frame_idx, study_state,
    ...
    # --- vvv PACKAGE DISCOVERY LOGIC (MODIFIED FOR VAGUE SKIP) vvv ---
        for pkg in current_packages:
            if pkg['id'].startswith('d'):
                ...
                if current_discovered and not prev_discovered:
                    ...
                    # FINAL MODIFICATION: Skip message if Discovered_by is N/A or empty
                    if discovered_by in ['Null', 'N/A', '', None]:
                        continue # Skip this vague discovery message

                    # Build the message text
                    message_text_list = []
                    if discovered_by != 'N/A' and discovered_by == assigned_to:
                         message_text_list = [
                             "Package ", html.Strong(pkg_id), f" discovered by {discovered_by} and it is en route to collect it." # Use discovered_by ID
                         ]
                    ...
                    if msg_id not in new_message_timestamps:
                        ...
                        if discovered_by.startswith('robot'):
                            ...
                            if 0 <= robot_index < 3:
                                new_robot_message_outputs[robot_index].insert(0, new_message_data)
    # --- ^^^ END MODIFICATION (Vague Discovery Skip) ^^^ ---
...
@app.callback(
    ...
    Output('responses-store', 'data', allow_duplicate=True),
    Output('study-state-store', 'data', allow_duplicate=True),
    Output('current-frame-store', 'data', allow_duplicate=True), # <-- ADD THIS LINE
...
def submit_post_scenario_questions(n_clicks, study_state, participant_data, responses, q_values, q_ids, interactions, all_message_logs):
    ...
    if new_study_state['current_run_idx'] < total_runs:
        ...
        return briefing_screen(next_run_idx + 1, total_runs, next_scenario_num, next_view_type, next_framework), new_responses, new_study_state, 0 # <-- ADD , 0
    else:
        # Finished all runs
...
""")
    ...
    app.run(debug=False, host='0.0.0.0', port=9521)
    # server = app.server