...
def create_figure_for_frame(static_data, frame_data):
    ...
    fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers', marker=dict(color='blue', size=10, symbol='circle'), name='Robot Waiting'))
...
def create_rich_status_message_data(robot_data, sim_time, all_packages, selected_robot_hmm_array, selected_robot_rmm_array, scenario_id):
    ...
    # --- NEW: Get Completed Packages ---

    completed_pkgs = robot_data.get('total_completed_packages', 0)

    

    time_status_text = f"Estimated Time to Goal: {eta_seconds} seconds."
    ...
    if robot_data.get('replan_flag') == True:
        ...
        details_msg.extend([
            ...
            f"Route recalculated.", *pkg_id_text, f" Robot is in the ", html.Strong(quadrant), " quadrant.",
        ...
        details_msg.extend([
            ...
            "Transporting ", html.Strong(pkg_id), f" to drop-off zone. Currently in the ", html.Strong(quadrant), " quadrant.",
        ...
        details_msg.extend([
            ...
            "Moving to acquire ", pkg_id, f". Currently in the ", html.Strong(quadrant), " quadrant.",
        ...
        details_msg.extend([
            ...
            f"Robot is awaiting task in the ", html.Strong(quadrant), " quadrant.",
        ...
        details_msg.extend([
            ...
            f"All tasks complete. Robot is in the ", html.Strong(quadrant), " quadrant.",
        ...
        details_msg.extend([
            ...
            f"Robot in unknown state '{state}' in the ", html.Strong(quadrant), " quadrant.",
    ...
    # --- NEW: Append ETA and Package Count to HEADER Text ---

    # This ensures it shows up in the summary

    final_status_text += f" | ETA: {eta_seconds}s | Pkgs: {completed_pkgs}/4"
    ...
    }, status_key, " ".join([str(item) for item in details_msg])
...
def update_simulation_views(frame_idx, study_state,
    ...
    if frame_idx is None or frame_idx >= len(scenario_data):
        ...
        return (
            ...
            current_fig, hist1 or [], hist2 or [], hist3 or [], 
        ...
        if robot_id in robots_dict:
            ...
            if framework_mode == 'with_framework':
                ...
                if selected_robot_hmm_array:
                    ...
                    if sync_occurred:
                        ...
                        any_sync_occurred_with_framework = True 
            ...
            if sync_occurred or generate_message:
                ...
                new_message_data, message_type, message_text_for_log = create_rich_status_message_data(
                    ...
                    robot_info, sim_time, packages, selected_robot_hmm_array, selected_robot_rmm_array, scenario_num
                ...
                if not is_duplicate:
                    ...
                    new_message_div = render_message_component(new_message_data, current_open_message_ids, is_new=is_new_message)
        ...
        for pkg in current_packages:
            ...
            if pkg['id'].startswith('d'):
                ...
                if current_discovered and not prev_discovered:
                    ...
                    if discovered_by in ['Null', 'N/A', '', None]:
...
@app.callback(
    ...
    Input('scenario-data-store', 'data'), 
    Input('study-state-store', 'data')
)
def update_snapshot(scenario_data, study_state): 
    ...
    # --- MODIFICATIONS FOR STATIC VIEW ---
    
    # 1. Filter out traces we don't want (Nodes causing black dots)
    #    and adjust text for packages.
    new_data = []
    for trace in static_fig.data:
        # Skip the 'Nodes' trace (usually color rgba(50, 50, 150, 0.8) or no name)
        # Based on your create_figure code, nodes are markers without a name property or named None in older versions
        # In your current code, nodes have showlegend=False and specific color.
        if hasattr(trace, 'marker') and isinstance(trace.marker.color, str) and '50, 50, 150' in trace.marker.color:
            continue # Skip nodes
            
        # Remove text from Package traces
        if hasattr(trace, 'name') and trace.name and 'Package' in trace.name:
            trace.mode = 'markers' # Force mode to markers only (hides text)
            trace.text = None      # Clear text data
            trace.hoverinfo = 'none' # Optional: clean up hover
            
        # Scaling down markers/lines for the smaller map
        if hasattr(trace, 'marker') and trace.marker is not None:
            current_size = trace.marker.size
            # Handle both scalar and list sizes
    ...
    static_fig.update_layout(
        ...
        title=None,
        font=dict(size=8),
        # Lock axis to grid to prevent distortion
