...
def update_simulation_views(frame_idx, study_state,
    ...
    for i in range(1, 4):
        ...
        if robot_id in robots_dict:
            ...
            # --- NEW: FORCE MESSAGE IF TASK COMPLETE ---
            # Regardless of framework mode or intervals, if the robot finished, we want to know.
            if robot_info.get('state') == 'all_tasks_complete':
                generate_message = True
            ...
            if sync_occurred or generate_message:
                ...
                if new_message_data is None:
                    ...
                    if current_robot_history:
                        ...
                        # RULE 1: Suppress consecutive 'on_track'
                        if current_core_state == 'on_track' and last_core_state == 'on_track':
                            if current_features == last_features:
                                should_send = False
                        
                        # RULE 2: Suppress consecutive 'deviation' (Timing Updates)
                        if current_core_state == 'deviation' and last_core_state == 'deviation':
                            if current_features == last_features:
                                should_send = False
                                
                        # RULE 3 (NEW): Suppress consecutive 'complete'
                        # If we already said "Mission Complete", don't say it again.
