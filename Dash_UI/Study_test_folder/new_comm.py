...
def create_rich_status_message_data(robot_data, sim_time, all_packages, selected_robot_hmm_array, selected_robot_rmm_array, scenario_id, framework_mode):
    ...
    # If ETA is 0 and we aren't finished, or if simulation just started
    ...
    # Track critical changes to force a message
    ...
    # Determine the "Core State" for suppression logic
    ...
    if state == 'all_tasks_complete':
        ...
        # If environment is bad, we escalate the header
        status_text = "ENVIRONMENT ALERT"
        status_icon = "⚠️"
        style_class = "stationary" # Use yellow/orange for awareness
        core_state = 'feature'
    elif framework_mode == 'with_framework':
        # If framework triggered this (and it's not the others), it's a timing deviation
        status_text = "TIMING UPDATE" 
        status_icon = "⏱️"
        style_class = "stationary"
        core_state = 'deviation'
    else:
        # Standard Baseline Message
    ...
    # Return the dictionary PLUS the core_state for the logic check
    # FIX: 'feature_set' is now a list (summary_icons) to be JSON serializable
    return {
        'message_id': message_id,
        ...
        'feature_set': summary_icons # <-- FIXED: Saving as list, not set
...
def update_simulation_views(frame_idx, study_state,
    ...
    for i in range(1, 4):
        ...
        if robot_id in robots_dict:
            ...
            # 1. Update HMM Logic (Always runs to keep mental model fresh)
            ...
            if sync_occurred or generate_message:
                # --- ADDED framework_mode ARGUMENT HERE ---
                ...
                if new_message_data is None:
                    ...
                    if current_robot_history:
                        ...
                        # Get states (default to 'unknown' if missing from history)
                        current_core_state = new_message_data.get('core_state', 'unknown')
                        last_core_state = last_msg.get('core_state', 'unknown')
                        
                        # FIXED: Convert the lists (from JSON) back to sets for comparison
                        current_features = set(new_message_data.get('feature_set', []))
                        last_features = set(last_msg.get('feature_set', []))
                        
                        # RULE: If both are "on_track" and features (icons) haven't changed -> SUPPRESS
                        if current_core_state == 'on_track' and last_core_state == 'on_track':
    ...
    # [Rest of the function: Package Assignments, Discoveries, etc. - UNCHANGED]
    if frame_idx > 0:
...
""")
    ...
    app.run(debug=False, host='0.0.0.0', port=8044)