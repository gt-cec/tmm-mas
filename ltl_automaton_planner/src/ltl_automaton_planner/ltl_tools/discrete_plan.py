...
def dijkstra_predecessor_and_distance(graph, start_node):
    ...
    while unvisited_nodes:
        ...
        for neighbor in graph.successors(current_min_node):
            tentative_value = shortest_path[current_min_node] + graph.edges[current_min_node, neighbor]["weight"]
    ...
    filtered_shortest_path = {key: value for key, value in shortest_path.items() if value != float('inf')}
    return previous_nodes, filtered_shortest_path
...
def dijkstra_targets(product, prod_source, prod_targets):
    ...
    while (tovisit and feasible_targets):
        ...
        d = dist[f_prod_node]
        for t_prod_node in product.successors(f_prod_node): #product.fly_successors(f_prod_node):
            cost = product.edges[f_prod_node, t_prod_node]["weight"]
            nd = d + cost 
        ...
        if f_prod_node in feasible_targets:
            ...
            return compute_path_from_pre(pre_node, f_prod_node), dist[f_prod_node], time.time()-start
...
def residual(run, state, segment="prefix"):
    res_cost = 0
    res_count = -1
    if segment == "prefix":
        for res in reversed(run.prefix):
            if res == state:
                return res_cost, res_count
            else:
                res_count -= 1
                res_cost += run.pre_plan_cost[-res_count]
    else:
        for res in reversed(run.suffix):
            if res == state:
                return res_cost, res_count
            else:
                res_count -= 1
                res_cost += run.suf_plan_cost[-res_count]
    return None, None
