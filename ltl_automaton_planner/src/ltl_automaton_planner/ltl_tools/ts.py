...
class TSModel(DiGraph):
    ...
    def compose_nodes(self, graph_list):
        ...
        for node in node_product:
            ...
            for i in range(len(graph_list)):
                label = label.union(graph_list[i].nodes[(node[i],)]['label'])
            
            self.add_node(node, label=label, marker='unvisited')
                    ...
                    for successor_state in graph_list[i].successors(state):
                        ...
                        if self.is_action_allowed(graph_list[i][state][successor_state]['guard'], self.nodes[node]['label']):
                            self.add_edge(node, successor_node,
                                          ...
                                          marker='visited')
                            # print(node, " ", successor_node)
                            # print(self.nodes[node]['label'])
                            # print(self.nodes[successor_node]['label'])
    ...
    def fly_predecessors(self, prod_node):
        ...
        for f_prod_node in self.predecessors(prod_node):
            if f_prod_node != prod_node:
                cost = self.edges[f_prod_node, prod_node]['weight']
